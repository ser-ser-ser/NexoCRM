-- Migration: 002_schema_espanol_v4.sql
...
-- 6. BACKFILL MISSING PROFILES (The Fix)
-- Insert ghost profiles for owners that exist in properties but not in perfiles
-- This assumes the user exists in auth.users. If not, this might still fail, 
-- but usually they do.
INSERT INTO perfiles (id, nombre_completo)
SELECT DISTINCT propietario_id, 'Usuario Migrado'
FROM propiedades
