-- Migration: 002_schema_espanol_v6.sql
...
ALTER TABLE propiedades DROP CONSTRAINT IF EXISTS fk_propiedad_propietario; -- Drop if partial failed attempt left it
...
-- 5. Migrate data from 'properties' to 'propiedades' (New rows)
...
SELECT 
  ...
  p.currency, 
  NULL, 
  CASE WHEN EXISTS (SELECT 1 FROM perfiles WHERE id = p.owner_id) THEN p.owner_id ELSE NULL END, 
  p.created_at
FROM properties p
ON CONFLICT (id) DO NOTHING;

-- 6. CLEANUP EXISTING BAD FKs (The Real Fix)
-- Update rows that already existed with bad IDs (orphaned)
UPDATE propiedades
SET propietario_id = NULL
WHERE propietario_id IS NOT NULL 
  AND propietario_id NOT IN (SELECT id FROM perfiles);
